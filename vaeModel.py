...
# =========================
# VAE Exposure Encoder
# =========================
class ExposureVAE(nn.Module):
    def __init__(self, num_items, latent_dim=32, hidden_dim=64):
        super().__init__()
...
# Training Functions
# =========================
def train_exposure_vae(model, exposure_data, num_epochs=50, lr=0.001):
    optimizer = optim.Adam(model.parameters(), lr=lr)

    for epoch in range(num_epochs):
        model.train()
        optimizer.zero_grad()
        recon, mu, logvar = model(exposure_data)
        loss = vae_loss(recon, exposure_data, mu, logvar)
        loss.backward()
        optimizer.step()
        if epoch % 5 == 0:
            print(f"[Exposure VAE] Epoch {epoch}, Loss: {loss.item():.2f}")
    return model
